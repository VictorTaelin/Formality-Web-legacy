...
const parse = (code) => {
  ...
  function parse_term(ctx) {
    ...
    // Lambda
      ...
      var skip = parse_exact("]");
      var body = parse_term(extend(ctx, [name, Var(0)]));
      return expr ? "[" + name + " = " + expr + "] " + body : (bind ? "[" + eras + name + " : " + bind + "] " + body : "[" + eras + name + "] " + body);
    }
    ...
    // Ann
      ...
      var skip = parse_exact("=");
      var expr = parse_term(ctx);
      console.log("\nI am in [Ann] matching ':'");
      console.log(expr);
      return show(expr, ctx);
    }
    ...
    // Variable / Reference
      ...
      if (var_index === null) {
        ...
        console.log("I am on the final [else] on else");
